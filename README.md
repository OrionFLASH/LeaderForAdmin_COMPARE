...
* **Excel-файл (`.xlsx`)** с листами:
  ...
  3. **COMPARE** — сравнение всех ключевых полей и статусные колонки
  ...
  7. **FINAL\_RAW, FINAL\_PLACE\_RAW** — служебные листы с промежуточными данными
...
3. **Сравнение выгрузок:**
   ...
   * `make_compare_sheet()` строит таблицу различий, заполняет статусные колонки (BANK, TB, GOSB, призовые категории).

4. **Построение финальных таблиц:**

   * `build_final_sheet_fast()` — определяет лучший призовой статус для сотрудника по каждому турниру.
   * `build_final_place_sheet_from_compare()` — определяет перемещения по местам (улучшение/ухудшение/новое/пропало).
...
* v1.2 — Итоговые RAW-листы, расширенная агрегация
